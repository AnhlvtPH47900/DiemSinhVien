namespace DiemSinhVien
{
    partial class Form1
    {
        /// <summary>
        ///  Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        ///  Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        ///  Required method for Designer support - do not modify
        ///  the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            label1 = new Label();
            label2 = new Label();
            txtUers = new TextBox();
            txtPass = new TextBox();
            btnLogin = new Button();
            btnCancel = new Button();
            grLogin = new GroupBox();
            label3 = new Label();
            comboBox1 = new ComboBox();
            grLogin.SuspendLayout();
            SuspendLayout();
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Location = new Point(43, 56);
            label1.Name = "label1";
            label1.Size = new Size(82, 20);
            label1.TabIndex = 0;
            label1.Text = "User Name";
            // 
            // label2
            // 
            label2.AutoSize = true;
            label2.Location = new Point(43, 103);
            label2.Name = "label2";
            label2.Size = new Size(70, 20);
            label2.TabIndex = 1;
            label2.Text = "Password";
            // 
            // txtUers
            // 
            txtUers.Location = new Point(131, 49);
            txtUers.Name = "txtUers";
            txtUers.Size = new Size(226, 27);
            txtUers.TabIndex = 2;
            // 
            // txtPass
            // 
            txtPass.Location = new Point(131, 96);
            txtPass.Name = "txtPass";
            txtPass.Size = new Size(226, 27);
            txtPass.TabIndex = 3;
            // 
            // btnLogin
            // 
            btnLogin.Location = new Point(99, 223);
            btnLogin.Name = "btnLogin";
            btnLogin.Size = new Size(94, 29);
            btnLogin.TabIndex = 4;
            btnLogin.Text = "Login";
            btnLogin.UseVisualStyleBackColor = true;
            btnLogin.Click += btnLogin_Click;
            // 
            // btnCancel
            // 
            btnCancel.Location = new Point(247, 223);
            btnCancel.Name = "btnCancel";
            btnCancel.Size = new Size(94, 29);
            btnCancel.TabIndex = 5;
            btnCancel.Text = "Cancel";
            btnCancel.UseVisualStyleBackColor = true;
            btnCancel.Click += btnCancel_Click;
            // 
            // grLogin
            // 
            grLogin.Controls.Add(label3);
            grLogin.Controls.Add(comboBox1);
            grLogin.Controls.Add(label1);
            grLogin.Controls.Add(btnCancel);
            grLogin.Controls.Add(txtUers);
            grLogin.Controls.Add(btnLogin);
            grLogin.Controls.Add(label2);
            grLogin.Controls.Add(txtPass);
            grLogin.Location = new Point(149, 81);
            grLogin.Name = "grLogin";
            grLogin.Size = new Size(450, 287);
            grLogin.TabIndex = 6;
            grLogin.TabStop = false;
            grLogin.Text = "Login";
            // 
            // label3
            // 
            label3.AutoSize = true;
            label3.Location = new Point(43, 168);
            label3.Name = "label3";
            label3.Size = new Size(39, 20);
            label3.TabIndex = 7;
            label3.Text = "Role";
            // 
            // comboBox1
            // 
            comboBox1.FormattingEnabled = true;
            comboBox1.Items.AddRange(new object[] { "Cán bộ đào tạo", "Giảng viên" });
            comboBox1.Location = new Point(131, 160);
            comboBox1.Name = "comboBox1";
            comboBox1.Size = new Size(226, 28);
            comboBox1.TabIndex = 6;
            comboBox1.SelectedIndexChanged += comboBox1_SelectedIndexChanged;
            // 
            // Form1
            // 
            AutoScaleDimensions = new SizeF(8F, 20F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(800, 450);
            Controls.Add(grLogin);
            Name = "Form1";
            Text = "Form1";
            grLogin.ResumeLayout(false);
            grLogin.PerformLayout();
            ResumeLayout(false);
        }

        #endregion

        private Label label1;
        private Label label2;
        private TextBox txtUers;
        private TextBox txtPass;
        private Button btnLogin;
        private Button btnCancel;
        private GroupBox grLogin;
        private Label label3;
        private ComboBox comboBox1;
    }
}